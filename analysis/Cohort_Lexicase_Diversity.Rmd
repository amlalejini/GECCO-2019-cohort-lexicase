...
all_cohort_data$PROG_COHORT_SIZE <- as.factor(all_cohort_data$PROG_COHORT_SIZE)
...
all_cohort_data_time_solved$PROG_COHORT_SIZE <- as.factor(all_cohort_data_time_solved$PROG_COHORT_SIZE)
```
...
```{r}
summary(lm(prog_gen_sys_current_phylogenetic_diversity/generation ~ as.numeric(PROG_COHORT_SIZE),data=all_cohort_data_time_solved %>% filter(PROG_SELECTION_MODE == 1)))
```
